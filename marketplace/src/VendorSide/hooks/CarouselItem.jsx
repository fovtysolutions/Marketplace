import React from "react";
import '../css/style.css';
import { IoDiamond } from "react-icons/io5";

const CarouselItem = ({ items = {} }) => {
    const rating = parseFloat(items.rating);
    return (
        <>
            <div className="carouselven-body">
                <p className="carouselven-badge">{items.badge}</p>
                <img src={items.image !== 'imageurl' ? items.image : "data:image/png;base64,"} alt="Item Image" className="carouselven-image" />
                <h4 className="carouselven-title">{items.title}</h4>
                <p className="carouselven-price">US {items.price}</p>
                <p className="carouselven-order">Min. Order: {items.order}</p>
                <p className="carouselven-origin">Origin: {items.origin}</p>
                <p className="carouselven-description">{items.description}</p>
                <p className="carouselven-rating">
                    {rating > 3.2 ? (
                        <>
                            <IoDiamond />
                            <IoDiamond />
                            <IoDiamond />
                        </>
                    ) : rating > 1.6 ? (
                        <>
                            <IoDiamond />
                            <IoDiamond />
                        </>
                    ) : (
                        <IoDiamond />
                    )}
                </p>
                <div className="carouselven-buttons">
                    <button className="carouselven-button">Contact Us Now</button>
                    <button className="carouselven-button">Add To Cart</button>
                </div>
            </div>
        </>
    );
};

export default CarouselItem;
